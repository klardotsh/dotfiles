...
find -L "$FLAC_DIR" -type d -printf "$LOSSY_DIR/%P\0" | XARGS mkdir -p

# Encode tracks
find -L "$FLAC_DIR" -type f -iname '*.flac' -printf "%P\0" | XARGS -I{} -n 1 bash -c 'encode "$@"' _ {}

# Copy covers
find -L "$FLAC_DIR" -type f -iregex "$COVER_REGEX" -printf "%P\0" |
	XARGS -I{} -n 1 bash -c 'folder_rename "$@"' _ {} |
	XARGS -n 2 cp --preserve -u

# Remove encodes if the original has been removed
comm -23 <(find -L "$LOSSY_DIR" -type f -iname '*.opus' -printf "%P\n" | sort) \
	<(find -L "$FLAC_DIR" -type f -iname '*.flac' -printf "%P\n" | sed 's/flac$/opus/' | sort) |
	XARGS -d '\n' -I{} rm "$LOSSY_DIR/{}"
